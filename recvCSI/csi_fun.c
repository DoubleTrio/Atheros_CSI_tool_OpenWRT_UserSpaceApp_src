...
void fill_csi_matrix(u_int8_t *csi_addr, int nr, int nc, int num_tones, COMPLEX(*csi_matrix)[3][114])
{
	...
	// for each real and imaginary value of the CSI matrix H 
	...
	for (k = 0; k < num_tones; k++)
	{
		...
		{ // Loop for each Rx antenna
			...
			{ 	// If total bits less than 10, then get next 16 bits
				if ((bits_left - 10) < 0)
				{
					h_data = csi_addr[idx++];
					h_data += (csi_addr[idx++] << 8);
...
{
	// Here is the function for csi processing and can call your own function */
	return;
}